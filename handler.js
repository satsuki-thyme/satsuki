...
function loadFiles() {
  ...
  return Promise.all([CSSPromiseArray, scriptPromiseArray])
}
        ...
        .then(async rly => {
          if (rly.ok) {
            ...
            if (fileType === `markdown`) {
              ...
              outputText = textForCopy = await novelparse({"src": w1, "newLineMode": newLineMode, "rubyMode": rubyMode, "parenthesis": `normal`, "commnet": `delete-together`})
            }
            ...
            .then(rly => {
              ...
              let relationLink = `<nav class="nav-outer"><div class="nav-inner"><table class="relation-link"><tbody><tr><td>${prevLink}</td><td><span class="nav-separater">|</span><a href="${basePage}?q=${dn}">表紙</a><span class="nav-separater">|</span></td><td>${nextLink}</tx></tr></tbody></table></div></nav>`
              ...
              let footer = `<footer>${relationLink}<p>文字数<span class="rot">:</span><span class="rot"> </span><span class="char-num">${rly[4][0]}</span></p><p id="copyright">&copy;<span class="rot"> </span>Satsuki<span class="rot"> </span>Thyme</p></footer>`
              ...
              for (let i in mark) {
                ...
                keywordList[i].description = await Promise.all(
                  ...
                  .map(async rly => {
                    ...
                    .replace(reMark, ``)
                    return (await novelparse({"src": (await brackettool(await brackettool(w0, marksPreposition, `delete-together`), marksEnclosure, `delete`)).replace(reMarklessKeyword, `<span class="mark">$1</span>`), "newLineMode": `unprocessed`, "rubyMode": `parse`, "parenthesis": `normal`, "comment": `delete-together`})).replace(/　 /, ``)
                  })
                )
